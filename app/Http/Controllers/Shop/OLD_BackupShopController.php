...
class ShopController extends Controller
{
    ...
    public function buyProductForm( Request $request, $pId = null, $pAmount = null )
    {
        ...
        if ( $request->getMethod() == 'POST' || $backAction ) {
            ...
            $totalPrice = $isBoxingProduct ? (($product->category->slug=='morty-nachnahme' || $product->category->slug=='welt-nachnahme' )?$product->price_in_cent/10:0) : ($product->price_in_cent * $amount);
    ...
    public function buyProductConfirmForm( Request $request )
    {
        ...
        if ($request->getMethod() == 'POST' && ! is_null($productId)) {
            ...
            if ( $amount > 0 && $product->isAvailableAmount($amount)) {
                ...
                //$priceInCent = $amount * ($isBoxingProduct ? ($request->input('product_payment_amount', 500) * 10) : $product->price_in_cent);
                ...
                if ( Auth::user()->balance_in_cent >= $priceInCent ) {
                    ...
                    if ( $product->isUnlimited() || $product->isDigitalGoods()) {
                        ...
                        if ($isBoxingProduct) {
                            ...
                            $orderDetails['total_price_in_btc'] = $this->bitcoinConverterService->toBitcoin(($orderDetails['product_payment_amount'] / 10), Setting::getShopCurrency());
                        }
