...
@section('content')
    ...
    <div class="k-content__body	k-grid__item k-grid__item--fluid">
        <form class="inline-form mb-4">
            <div class="row">
                <div class="col-lg-6">
                    <input type="text" name="term" value="{{ $term }}" class="form-control"
                           placeholder="Schlüsselwörter: Benutzername, Bestell-ID, DHL-Tracking-Nummer"/>
                </div>
                <div class="col-lg-1">
            ...
            <ul class="navbar-nav mr-auto">
                <li class="nav-item">
                    <form action="{{ route( 'backend-orders-with-pageNumber', 1 ) }}" method="get" class="row">
                        ...
                        <button type="submit"
                                class="btn btn-brand mx-4">{{ __('backend/orders.fiters.fiter') }}</button>
                    </form>
                </li>
                ...
                <li class="nav-item">
                    <form action="{{ route( 'backend-orders-clearFilter' ) }}" method="post" class="row">
                        @csrf
                        <button type="submit"
                                class="btn btn-brand mx-4">{{ __('backend/orders.fiters.clear_filter') }}</button>
                    </form>
                </li>
                ...
                <li class="nav-item">
                    <form action="{{ route( 'backend-orders-with-pageNumber', 1 ) }}" method="get" class="row">
                        ...
                        <button type="submit"
                                class="btn btn-brand mx-4">{{ __('backend/orders.fiters.today_oders') }}</button>
                    </form>
                </li>
                ...
                <li class="nav-item">
                    <form action="{{ route( 'address-not-changed', 1 ) }}" method="get" class="row">
                        ...
                        <button type="submit"
                                class="btn btn-brand mx-4">{{ __('backend/orders.fiters.address_not_changed') }}</button>
                    </form>
                </li>
                <li class="nav-item">
                    <a href="javascript:void(0);" id="bulk-action-btn" onclick="submitForm();" style="display:none;"
                       class="btn btn-brand mx-4">Massenstatusänderung</a>
                </li>
            </ul>
        </div>
        ...
        <ul class="nav nav-tabs">
            ...
            @endif
            @if(Auth::user()->hasPermission('manage_orders') || Auth::user()->hasPermission('manage_orders_100'))
                <li class="nav-item">
                    <a class="nav-link" data-toggle="tab" href="#percent-100">100%</a>
                </li>
            @endif
                @if(Auth::user()->hasPermission('manage_orders') || Auth::user()->hasPermission('manage_orders_80'))
                    <li class="nav-item">
                        <a class="nav-link" data-toggle="tab" href="#percent-80">Originale 80%</a>
                    </li>
                @endif
        </ul>
        ...
        <div class="tab-content">
            @if(Auth::user()->hasPermission('manage_orders') || Auth::user()->hasPermission('manage_orders_packstation'))
                <div class="tab-pane container active" id="packing_station" style="max-width: 100%">
                    @component('components.backend.orders.order_table', [ 'orders' => $orders_packing_station, 'download' => true ])@endcomponent
                </div>
            @endif
            @if(Auth::user()->hasPermission('manage_orders') || Auth::user()->hasPermission('manage_orders_filialeinlieferung'))
                <div class="tab-pane container" id="branch_delivery" style="max-width: 100%">
                    @component('components.backend.orders.order_table', [ 'orders' => $orders_branch_delivery, 'download' => true ])@endcomponent
                </div>
            @endif
            @if(Auth::user()->hasPermission('manage_orders') || Auth::user()->hasPermission('manage_orders_lit_filling'))
                <div class="tab-pane container" id="lit_filling" style="max-width: 100%">
                    @component('components.backend.orders.order_table', [ 'orders' => $ordersLitFilling ?? [], 'download' => true ])@endcomponent
                </div>
            @endif
            @if(Auth::user()->hasPermission('manage_orders') || Auth::user()->hasPermission('manage_orders_lit_refund'))
                <div class="tab-pane container" id="lit_refund" style="max-width: 100%">
                    @component('components.backend.orders.order_table_refund', [ 'orders' => $ordersLitRefund ?? [], 'download' => true ])@endcomponent
                </div>
            @endif
            @if(Auth::user()->hasPermission('manage_orders') || Auth::user()->hasPermission('manage_orders_nachnahme'))
                <div class="tab-pane container" id="nachnahme" style="max-width: 100%">
                    @component('components.backend.orders.order_table_nachnahme', [ 'orders' => $ordersBoxing ?? [], 'download' => true ])@endcomponent
                </div>
            @endif
            @if(Auth::user()->hasPermission('manage_orders') || Auth::user()->hasPermission('manage_orders_random'))
                <div class="tab-pane container" id="random" style="max-width: 100%">
                    @component('components.backend.orders.order_table_random', [ 'orders' => $ordersRandom ?? [], 'download' => true ])@endcomponent
                </div>
            @endif
            @if(Auth::user()->hasPermission('manage_orders') || Auth::user()->hasPermission('manage_orders_accounts'))
                <div class="tab-pane container" id="accounts" style="max-width: 100%">
                    @component('components.backend.orders.order_table_accounts', [ 'orders' => $ordersAccounts ?? [], 'download' => true ])@endcomponent
                </div>
            @endif
            @if(Auth::user()->hasPermission('manage_orders') || Auth::user()->hasPermission('manage_orders_100'))
                <div class="tab-pane container" id="percent-100" style="max-width: 100%">
                    @component('components.backend.orders.order_table_100', [ 'orders' => $orders100percents ?? [] ])@endcomponent
                </div>
            @endif
            @if(Auth::user()->hasPermission('manage_orders') || Auth::user()->hasPermission('manage_orders_80'))
                <div class="tab-pane container" id="percent-80" style="max-width: 100%">
                    @component('components.backend.orders.order_table_100', [ 'orders' => $orders80percents ?? [] ])@endcomponent
                </div>
            @endif
        </div>
