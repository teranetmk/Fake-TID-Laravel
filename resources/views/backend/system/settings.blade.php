													...
													<div class="kt-portlet__body">
														<div class="kt-section kt-section--first">
															...
															<div class="form-group">
																...
																<input type="text" class="form-control @if($errors->has('settings_api_key')) is-invalid @endif" id="settings_api_key" name="settings_api_key" placeholder="{{ __('backend/system.settings.api_key') }}" value="@if(strlen(App\Models\Setting::get('api.key')) > 0){{ App\Models\Setting::get('api.key') }}@endif" />
															...
															<div class="form-group">
																<label for="sold_out_products">Produkte als Ausverkauft markieren:</label>
																@foreach(App\Models\Product::all() as $product)
																<div id="sold_out_products">
																	<label class="k-checkbox k-checkbox--all k-checkbox--solid">
																		<input type="checkbox" name="sold_out_products[]" value="{{ $product->id }}" @if($product->isSoldOut()) checked @endif />
																		<span></span>
																		{{ $product->name }}
																	</label>
																</div>
																@endforeach
																@if($errors->has('sold_out_products'))
																	<span class="invalid-feedback" style="display:block;" role="alert">
																		<strong>{{ $errors->first('sold_out_products') }}</strong>
																	</span>
																@endif
															</div>
															
															<div class="form-group">
																<label for="app_mode">Anwendungsmodus:</label>
																@foreach(BADDIServices\Framework\App::APP_MODES as $mode)
																<div id="app_mode">
																	<label class="k-checkbox k-checkbox--all k-checkbox--solid">
																		<input type="radio" name="app_mode" value="{{ $mode }}" @if($mode == $appMode) checked @endif />
																		<span></span>
																		{{ $mode }}
																	</label>
																</div>
																...
																@if($errors->has('app_mode'))
																	<span class="invalid-feedback" style="display:block;" role="alert">
																		<strong>{{ $errors->first('app_mode') }}</strong>
																	</span>
																@endif
															</div>
														</div>
