                            ...
                            @foreach($orders as $order)
                                <tr>
                                    ...
                                    <td style="font-size: 20px;">
                                        <?php $edit_btn = $order->random_shipping_address; ?>
                                        @if(! is_null($order->tids) && ! is_null($edit_btn) && Storage::disk('public')->exists("order/{$order->id}/{$order->tids->tid}"))
                                        ...
                                        <a href="{{ route('backend-order-edit', $order->id) }}"
                                           ...
                                           data-original-title="{{ __('backend/orders.edit') }}"><i
                                                class="la la-edit @if($edit_btn->recipient_first_name) text-success @else text-danger @endif"></i></a>
                                        @endif
                                        {{--
