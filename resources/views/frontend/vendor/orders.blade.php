                    ...
                    @foreach($user_orders as $order)
                        ...
                        <div class="card mb-15 bg-shahzad-lightgray" id="orderHeading-{{ $loop->iteration }}">
                            <div class="card-header header-colour-none p-3">
                                <h5 class="mb-0">
                                    <button class="btn btn-link btn-block text-left text-decoration-none collapsed" data-toggle="collapse" data-target="#orderCollapse-{{ $loop->iteration }}" aria-expanded="@if($loop->iteration == 1) true @else false @endif" aria-controls="orderCollapse-{{ $loop->iteration }}">
                            ...
                            <div id="orderCollapse-{{ $loop->iteration }}" class="collapse @if($loop->iteration == 1) show @endif" aria-labelledby="orderHeading-{{ $loop->iteration }}" data-parent="#orderAccordion">
                                ...
                                <ul class="list-group list-group-flush">
                                    ...
                                    <li class="list-group-item">
                                        <b>{{ __('frontend/shop.price') }}</b> {{ ($order->products->name !== 'Nachnahme Boxing') ? $order->getFormattedPrice() : ((number_format($order->product_payment_amount / 10, 2, ',', '.') . ' ' . \App\Models\Setting::getShopCurrency())) }}
                                    </li>
                                    @endif
...
{{--                                    @endif--}}
                                    ...
                                    @if(!$order->products->isDigitalGoods())
                                        ...
                                        <li class="list-group-item">
                                            ...
                                            {{ __(sprintf('frontend/shop.orders.status.%s', $order->getStatus() ?? 'pending')) }}
                                        </li>
                                        @endif
                                    @endif
                                    ...
                                    <li class="list-group-item">
									    ...
									    @elseif($order->boxing_status == 'package_was_refused')
                                            Paketannahme wurde verweigert, kontaktiere uns bitte für einen Replace.
                                        @else
                                            ---
										@endif
                                    </li>
								...
								@if(! is_null($order->products) && $order->products->name !== 'Nachnahme Boxing' && isset($order->tids))
                                    <li class="list-group-item">
								...
								@endif
                                    ...
                                    <li class="list-group-item">
                                        ...
                                        @if(\App\Models\TidPackStation::where('tid_id', $order->tids->id)->first())
                                        <a href="{{ route('receipt.jpg', ['tidId' => $order->tids->id]) }}" target="_blank">{{ __('backend/orders.table.download') }}</a>
                                        @else
                                    ...
                                    <li class="list-group-item">
                                        <b>BTC Wert bei Zeit der Bestellung:</b> {{ $order->total_price_in_btc ?? 0 }}฿
                                    </li>
                                    <li class="list-group-item">
