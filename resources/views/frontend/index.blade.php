                ...
                <div id="newsAccordion" class="accordion-with-icon">
                    ...
                    <div class="card mb-15" id="newsHeading-{{ $loop->iteration }}">
                        <div class="card-header">
                            <h5 class="mb-0">
                                <button class="btn btn-link btn-block text-left text-decoration-none" data-toggle="collapse" data-target="#newsCollapse-{{ $loop->iteration }}" aria-expanded="@if($loop->iteration == 1) true @else false @endif" aria-controls="newsCollapse-{{ $loop->iteration }}">
                                    {{ $article->title }}
                                </button>
                            </h5>
                        </div>
                        ...
                        <div id="newsCollapse-{{ $loop->iteration }}" class="collapse @if($loop->iteration == 1) show @endif" aria-labelledby="newsHeading-{{ $loop->iteration }}" data-parent="#newsAccordion">
                            <div class="card-body">
                                {!! strlen($article->body) > 0 ? nl2br($article->body) : '' !!}
                            </div>
                        </div>
